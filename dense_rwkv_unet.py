...
# Define the DenseLayer with Spatial Mix
class Dense_RWKV_Layer(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, act_layer, drop_path=0., drop=0., channel_gamma=1/4, shift_pixel=1,attn_s=True):
        super(Dense_RWKV_Layer, self).__init__()

        self.norm = get_norm(norm_layer)(in_channels)
        self.conv1 = ConvNormAct(in_channels, out_channels, kernel_size=1)
        # self.spatial_mix = VRWKV_SpatialMix(out_channels, channel_gamma, shift_pixel)
        self.conv2 = ConvNormAct(out_channels, out_channels, kernel_size=3, stride=1, groups=out_channels)
        self.drop_path = DropPath(drop_path) if drop_path else nn.Identity()
        self.proj = ConvNormAct(out_channels, out_channels, kernel_size=1)
        self.proj_drop = nn.Dropout(drop)
        self.conv3 = nn.Conv2d(in_channels,out_channels, kernel_size = 1, stride = 1)
        if attn_s==True:
            self.att = VRWKV_SpatialMix(out_channels, channel_gamma, shift_pixel)
        self.attn_s=attn_s
    ...
    def forward(self, x):
        ...
        if self.attn_s:
            ...
            x = x.permute(0, 2, 1)
            # print(x.shape)
            x = self.att(x, patch_resolution) # permute(0, 3, 1, 2)
        ...
        # x = torch.cat([shortcut,self.drop_path(x)],dim=1)
        x = shortcut + self.drop_path(x)
        return x

# Define the DenseBlock with Spatial Mix
class DenseBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers, norm_layer, act_layer, drop_path=0., drop=0., channel_gamma=1/4, shift_pixel=1, if_no_zero= True, attn_s=True):
        ...
        for i in range(num_layers):
            ...
            tmp_channels = in_channels + i*out_channels
            layer = Dense_RWKV_Layer(tmp_channels, out_channels, norm_layer, act_layer, drop_path, drop, channel_gamma, shift_pixel,attn_s=True)
            self.layers.append(layer)
        if if_no_zero:
            self.conv = nn.Conv2d(out_channels * num_layers + in_channels, out_channels, kernel_size = 3, stride =2, padding = 1)
        else:
            self.conv = nn.Conv2d(out_channels * num_layers + in_channels, out_channels, kernel_size = 1, stride =1)
...
class RWKV_UNet_encoder(nn.Module):
    def __init__(self, dim_in=3, num_classes=1000, img_size=224,
        ...
        #     )
        # ])
        # self.out_list = [dim_in + stem_dim]
        self.conv_head = nn.Conv2d(dim_in, stem_dim , kernel_size = 1, stride = 1)

        self.stage0 = nn.ModuleList([
            DenseBlock(stem_dim, stem_dim, 1, norm_layer=norm_layers[0], act_layer=act_layers[0], drop_path=0., drop=0., channel_gamma=1/4, shift_pixel=1,if_no_zero = False, attn_s=True)
        ])
        img_size=img_size//2
        emb_dim_pre = stem_dim

        # emb_dim_pre = self.out_list[0]
        for i in range(len(depths)):
            ...
            # for j in range(depths[i]):
                ...
                # print(emb_dim_pre)
                # layers.append(
                #     DenseBlock(emb_dim_pre, embed_dims[i],depths[i], norm_layer=norm_layers[i], act_layer = act_layers[i], drop_path=0., drop=0., channel_gamma=1/4, shift_pixel=1,attn_s=True) #  attn_ss[i],   
                # )
                # self.out_list.append(embed_dims[i])
                # # emb_dim_pre = embed_dims[i]
                # emb_dim_pre = sum(self.out_list)
            # self.out_list.append(embed_dims[i])
            self.__setattr__(f'stage{i + 1}', nn.ModuleList([DenseBlock(emb_dim_pre, embed_dims[i], depths[i], norm_layer=norm_layers[i], act_layer = act_layers[i], drop_path=0., drop=0., channel_gamma=1/4, shift_pixel=1,attn_s=True)]))
            # emb_dim_pre = sum(self.out_list)
            emb_dim_pre = embed_dims[i]
...
def RWKV_UNet_encoder_T(pretrained=False, **kwargs):
    model = RWKV_UNet_encoder(
        ...
        depths=[2, 2, 4, 2], stem_dim=16, embed_dims=[32, 48, 96, 160], exp_ratios=[2., 2.5, 3.0, 3.5],
        norm_layers=['bn_2d', 'bn_2d', 'ln_2d', 'ln_2d'], act_layers=['silu', 'silu', 'gelu', 'gelu'], dw_kss=[5, 5, 5, 5], 
        attn_ss=[False, False, True, True],drop=0., drop_path=0.05,**kwargs)
...
class RWKV_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        fft_x_real_imag = fft_x_real_imag.reshape(x.shape[0], -1, x.shape[2], x.shape[3])
        x = torch.cat([input, fft_x_real_imag], dim=1)
        print("fft_cat:",x.shape)
        # x = x.repeat(1, 3, 1, 1)
        # print("initial:",x.shape)
...
class RWKV_UNet_T(nn.Module):
    ...
    def forward(self, x):
        ...
        fft_x_real_imag = torch.view_as_real(fft_x)
        fft_x_real_imag = fft_x_real_imag.permute(0, 1, 4, 2, 3)
        fft_x_real_imag = fft_x_real_imag.reshape(x.shape[0], -1, x.shape[2], x.shape[3]) # B 2 H W 
        x = torch.cat([input, fft_x_real_imag], dim=1)
        # x.shape B 3 H W
...
if __name__ == "__main__":
    ...
    restored=torch.ones((1, 1, 128, 128)).type(torch.FloatTensor).cuda()
    # model = RWKV_UNet(in_channels=1, img_size= 128 , num_classes= 1 ,pretrained_path= None)
    model = RWKV_UNet_T(in_channels=1, img_size= 128 , num_classes= 1 ,pretrained_path= None)
    model.cuda() 
    
    since = time.time()
    y=model(x)
    loss_G = criterion(restored, y) 
    # print(loss_G)
    loss_G.backward()
